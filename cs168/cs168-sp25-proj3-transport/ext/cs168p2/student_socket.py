...
class StudentUSocket(StudentUSocketBase):
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      ## Start of Stage 1.3 ##
      self.rcv.nxt = seg.seq |PLUS| 1 # why plus 1? SYN-ACK takes up 1 segment space
      self.snd.una = seg.ack # no need for plus *anything*, since seg.ack already calculated that
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4.3 ##
    while self.tx_data:
      right = snd.una |PLUS| snd.wnd

      if right |LE| snd.nxt:
          break

      window_left = right |MINUS| snd.nxt

      if window_left == 0:
          break

      to_send = min(self.mss, window_left, len(self.tx_data))

      payload = self.tx_data[:to_send]

      packet = self.new_packet(ack=True, data=payload, syn=False)
      self.tx(packet)

      self.tx_data = self.tx_data[to_send:]
